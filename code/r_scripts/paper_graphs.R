...
brokenBoxPlots <- function(dfx, coly, amt
  ...
  pblapply(seq_along(dfnewSplit), function(xall) {
    ...
    # remove 11 ensemble size if carbon, due to only having one result
    if(es == "Carb"){
...
if(mainFigures){
  ...
  summExitResults <- pblapply(summExitGroups, function(x){
    x2 <- x %>% dplyr::select(-res) %>%
      ...
      # get absolute differences between each ensemble size for medians
      mutate(abs_dif_med = median_value - lag(median_value)
             # get differences between each ensemble size for medians (in %)
             , pc_dif_med = (median_value - lag(median_value)) / lag(median_value) * 100
             # get cumulative differences between each ensemble size for medians (in %)
      ...
      for(contx in conts){
        ...
        ppMaxList <- lapply(1:ESmax, function(i){
          ...
          normGraphsPC <- lapply(currentPerc, function(percx){
            ...
            # graph label
            gLabel <- paste0("ensemble size: ", i, " [", fxx, "]")
    ...
    for(iES in allES){
      ...
      for(i in 1:ESmax){
        ...
        patchOut <- (normGraphsPC[[1]] + guides(fill = "none") + theme(title = element_blank()) | 
          ...
          plot_annotation(title = paste0(esName, " & Species richness in ", iCont, "\n(ensemble size: ", i, " | max n: ", fxx, ")"),
    ...
    if(xall == 1){
      ...
      txt <- strwrap(
        paste(
          ...
          , "\nThe maps and graphs in this PDF were generated by overlaying ensembles of ecosystem services (ES) and biodiversity (BD) and observing where 'hotspots' of both spatially coincided. Model outputs related to ES and BD across different continents were obtained, with ES outputs representing carbon storage (Carbon), water supply (Water), and recreation (Recreation), while BD outputs always represented vascular plant species richness (Species richness).
...
\nThe results of all the same ensemble size category were summed and then normalised, creating the resulting maps below. '1' was equivalent to 100% of ensemble size ensembles ES and BD comparisons completely overlapping, this is known as 'Spatial Agreement'. '1' is indicated as either green, blue, or orange (for carbon, water, and recreation, respectively) on the maps, while '0' (i.e., never overlapped) is grey. The red number in the topright corner of each map indicates the percentile that was used as the cut-off. 
